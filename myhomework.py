...
def lessonslist(message, isitadmin):
    ...
    for i in alllessons:
        if i == 'АНГЛ ЯЗ':
            ...
            obj = cursor.fetchall()
            for obj0 in obj:
                onelesson += f'_*{a}) АНГЛ ЯЗ(Лабаченя):*_  {str(obj0)[2:len(str(obj0)) - 3]}\n-\n'
            cursor.execute(f"""SELECT text From `{maindb}`.`object` WHERE (`object` = 'ХАДАРОВИЧ')""")
            obj = cursor.fetchall()
            for obj0 in obj:
                onelesson += f'_*{a}) АНГЛ ЯЗ(ХАДАРОВИЧ):*_  {str(obj0)[2:len(str(obj0)) - 3]}\n-\n'
                a += 1
        elif i == 'БЕЛАРУСИ':
            cursor.execute(f"""SELECT text From `{maindb}`.`object` WHERE (`object` = 'БЕЛАРУСИ')""")
            obj = cursor.fetchall()
            for obj0 in obj:
                onelesson += f'_*{a}) ИСТОРИЯ БЕЛАРУСИ:*_  {str(obj0)[2:len(str(obj0)) - 3]}\n-\n'
            a += 1
        elif i == 'ВСЕМИРНАЯ':
            cursor.execute(f"""SELECT text From `{maindb}`.`object` WHERE (`object` = 'ВСЕМИРНАЯ')""")
            obj = cursor.fetchall()
            for obj0 in obj:
                onelesson += f'_*{a}) ВСЕМИРНАЯ ИСТОРИЯ:*_  {str(obj0)[2:len(str(obj0)) - 3]}\n-\n'
            a += 1
        elif i == 'ИНФОРМ':
            cursor.execute(f"""SELECT text From `{maindb}`.`object` WHERE (`object` = 'Ковалевская')""")
            obj = cursor.fetchall()
            for obj0 in obj:
                onelesson += f'_*{a}) ИНФОРМ(Ковалевская):*_  {str(obj0)[2:len(str(obj0)) - 3]}\n'
            cursor.execute(f"""SELECT text From `{maindb}`.`object` WHERE (`object` = 'Боркун')""")
            obj = cursor.fetchall()
            for obj0 in obj:
                onelesson += f'_*{a}) ИНФОРМ(Боркун):*_  {str(obj0)[2:len(str(obj0)) - 3]}\n-\n'
                a += 1
        else:
            ...
            for obj0 in obj:
                onelesson += f'_*{a}) {i}:*_  {str(obj0)[2:len(str(obj0)) - 3]}\n-\n'
                a += 1
